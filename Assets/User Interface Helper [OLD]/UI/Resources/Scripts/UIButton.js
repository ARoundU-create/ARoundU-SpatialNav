...
script.api.anchorPosition = null; // Set by AnchorPlacementController
script.api.camera = null;         // Set by AnchorPlacementController
script.api.arrowPrefab = null;    // Optional: prefab to spawn arrows

script.api.allowTouchEvents = !script.disableTouchEvents;

script.api.startNavigation = function () {
    const start = script.api.camera.getTransform().getWorldPosition();
    const end = script.api.anchorPosition;

    if (!start || !end) {
        print("❌ Missing camera or anchor position");
        return;
    }

    print(`🧭 Navigating from camera to anchor @ (${end.x.toFixed(2)}, ${end.y.toFixed(2)}, ${end.z.toFixed(2)})`);

    const count = 10;
    const dir = end.sub(start).uniformScale(1 / count);
    for (let i = 1; i <= count; i++) {
        const pos = start.add(dir.uniformScale(i));
        const arrow = script.api.arrowPrefab.instantiate(script.getSceneObject().getParent());
        arrow.getTransform().setWorldPosition(pos);
        arrow.getTransform().setLocalScale(new vec3(3, 3, 3));
    }
};
...
function pressDown() {
   ...
   if (script.api && script.api.startNavigation) {
    script.api.startNavigation();
}
}
